const Article = ({ title, text, tags, image, alt }) => {
    return <div className="alura-card">
        ...
        <div className="w-full flex-row justify-end gap-2 pr-5 hidden sm:flex ">
            {
                tags.map(tag => <span key={tag} className="bg-alura-100 dark:bg-dark-100 px-4 py-1 rounded-full text-gray-200 font-bold text-xs uppercase hover:bg-alura-200 hover:scale-110 hover:bg-gradient-to-r hover:from-alura-200">{tag}</span>)
            }
